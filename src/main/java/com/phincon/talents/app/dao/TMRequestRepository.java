...
public interface TMRequestRepository extends PagingAndSortingRepository<TMRequest,Long>{
	...
	@Query("select p from TMRequest p where p.company=:company AND p.employment=:employment AND LOWER(p.module)=LOWER(:module) AND LOWER(p.categoryType)=LOWER(:categoryType) AND LOWER(p.type)=LOWER(:type) AND p.startDate=:startDate")
