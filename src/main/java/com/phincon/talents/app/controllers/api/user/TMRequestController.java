...
import com.phincon.talents.app.dto.TMRequestTypeDTO;
...
public class TMRequestController {
	...
	public ResponseEntity<List<RequestCategoryType>> getRequestType(
		...
		if(assignment != null) {
			...
			List<RequestCategoryType> listCategory = requestCategoryTypeRepository.findByCompany(user.getCompany());
			
			// Load List RequestType by Company
			List<RequestType> listRequestTypes = requestTypeRepository.findByCompany(user.getCompany());
			for (RequestCategoryType requestCategoryType : listCategory) {
				// find request type based on criteria and added to listRequestTypesSelected
				int grade = 0;
				if(assignment.getGradeNominal() != null)
					grade = assignment.getGradeNominal();
				
				addRequestTypeToCategory(requestCategoryType, listRequestTypes, grade,assignment.getPositionName());
		...
		return new ResponseEntity<List<RequestCategoryType>>(listCategorySelected, HttpStatus.OK);
	}
	...
	private void addRequestTypeToCategory(
		...
		for (RequestType requestType : listRequestTypes) {
			if(requestCategoryType.getId() == requestType.getRequestCategoryType()) {
				boolean added = false;
				if(requestType.getGrade()!= null && requestType.getGrade() >0 ){
				...
				if(requestType.getPosition() != null && !requestType.getPosition().equals("")) {
					if(position != null){
	...
	public ResponseEntity<List<TMRequestHeader>> listRequest(
		...
		if (module == null) {
			listRequest = tmRequestHeaderRepository.findByCompanyAndEmployee(user.getCompany(), user.getEmployee());
		} else {
			listRequest = tmRequestHeaderRepository.findByCompanyAndEmployeeAndModule(user.getCompany(),
					user.getEmployee(), module);
		}


		return new ResponseEntity<List<TMRequestHeader>>(listRequest, HttpStatus.OK);
	}
	...
	public ResponseEntity<List<TMRequestHeader>> listNeedReport(
		...
		List<TMRequestHeader> listRequest = tmRequestHeaderRepository.findByCompanyAndEmployeeAndModuleAndNeedReport(user.getCompany(), user.getEmployee(), module);

		if (module == null) {
			listRequest = tmRequestHeaderRepository.findByCompanyAndEmployeeAndNeedReport(user.getCompany(), user.getEmployee());
		} else {
			listRequest = tmRequestHeaderRepository.findByCompanyAndEmployeeAndModuleAndNeedReport(user.getCompany(), user.getEmployee(), module);
		}
